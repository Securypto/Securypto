...
def setup():
    ...
    if not os.path.isdir('gitian.sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/Securypto-Project/gitian.sigs.git'])
    if not os.path.isdir('Securypto-detached-sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/Securypto-Project/Securypto-detached-sigs.git'])
    if not os.path.isdir('gitian-builder'):
        subprocess.check_call(['git', 'clone', 'https://github.com/devrandom/gitian-builder.git'])
    if not os.path.isdir('Securypto'):
        subprocess.check_call(['git', 'clone', 'https://github.com/Securypto-Project/Securypto.git'])
...
def build():
    ...
    os.makedirs('Securypto-binaries/' + args.version, exist_ok=True)
    ...
    subprocess.check_call(['make', '-C', '../Securypto/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    if args.linux:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../Securypto/contrib/gitian-descriptors/gitian-linux.yml'])
        print('\nVerifying v'+args.version+' Linux\n')
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../Securypto/contrib/gitian-descriptors/gitian-linux.yml'])

    if args.windows:
        print('\nVerifying v'+args.version+' Windows\n')
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-unsigned', '../Securypto/contrib/gitian-descriptors/gitian-win.yml'])
        if args.sign:
            print('\nVerifying v'+args.version+' Signed Windows\n')
            subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-signed', '../Securypto/contrib/gitian-descriptors/gitian-win-signer.yml'])

    if args.macos:
        print('\nVerifying v'+args.version+' MacOS\n')
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-unsigned', '../Securypto/contrib/gitian-descriptors/gitian-osx.yml'])
        if args.sign:
            print('\nVerifying v'+args.version+' Signed MacOS\n')
            subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-signed', '../Securypto/contrib/gitian-descriptors/gitian-osx-signer.yml'])
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/Securypto-Project/Securypto', help='Specify the URL of the repository. Default is %(default)s')
    ...
    if args.pull:
        ...
        os.chdir('../gitian-builder/inputs/Securypto')
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
