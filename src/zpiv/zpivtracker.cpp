...
CoinWitnessData* CzSCUTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSCUTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
void CzSCUTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSCUWallet* zSCUWallet)
{
    bool iszSCUWalletInitialized = (NULL != zSCUWallet);
    CMintMeta meta;
    ...
    if (! iszSCUWalletInitialized)
        zSCUWallet = new CzSCUWallet(strWalletFile);
    meta.isSeedCorrect = zSCUWallet->CheckSeed(dMint);
    if (! iszSCUWalletInitialized)
        delete zSCUWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
std::set<CMintMeta> CzSCUTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSCUWallet* zSCUWallet = new CzSCUWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zSCUWallet);
        }
        delete zSCUWallet;
