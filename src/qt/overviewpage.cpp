                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in securypto.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in securypto.conf");
    }
    ...
    bool showWatchOnlySCUAvailable = showSCUAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ...
    ui->labelWatchAvailable->setVisible(showWatchOnlySCUAvailable && showWatchOnly);

    // SCU Pending
    bool showSCUPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlySCUPending = showSCUPending || watchUnconfBalance != 0;
    ...
    ui->labelWatchPending->setVisible(showWatchOnlySCUPending && showWatchOnly);

    // SCU Immature
    bool showSCUImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showSCUImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showSCUImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showSCUImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // SCU Locked
    bool showSCULocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlySCULocked = showSCULocked || nWatchOnlyLockedBalance != 0;
    ...
    ui->labelWatchLocked->setVisible(showWatchOnlySCULocked && showWatchOnly);
    ...
    ui->labelSCUPercent->setVisible(showPercentages);
