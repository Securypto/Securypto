...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSecuryptoAmount"))
        settings.setValue("nAnonymizeSecuryptoAmount", 1000);

    nAnonymizeSecuryptoAmount = settings.value("nAnonymizeSecuryptoAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSecuryptoAmount"))
        SoftSetArg("-anonymizesecuryptoamount", settings.value("nAnonymizeSecuryptoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in securypto.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSecuryptoAmount:
            return QVariant(nAnonymizeSecuryptoAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeSecuryptoAmount:
            nAnonymizeSecuryptoAmount = value.toInt();
            settings.setValue("nAnonymizeSecuryptoAmount", nAnonymizeSecuryptoAmount);
            emit anonymizeSecuryptoAmountChanged(nAnonymizeSecuryptoAmount);
            break;
        case CoinControlFeatures:
