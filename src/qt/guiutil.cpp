...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Securypto address (e.g. %1)").arg("SerWHnyvY9b8XvuBtqWhoVkVcBj9vyHXgV"));
...
{
    // return if URI is not valid or is no Securypto: URI
...
{
    // Convert securypto:// to securypto:
    //
    //    Cannot handle this later, because securypto:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open securypto.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Securypto.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Securypto.lnk
...
{
    return GetAutostartDir() / "securypto.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a securypto.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Securypto\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the securypto app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add securypto app to startup item list
