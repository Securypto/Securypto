...
#include "config/securypto-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("securypto-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "securypto.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "securyptod.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 19997, 19998));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, securypto, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Securypto specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zSCU spends and stakes (0-1, default %u)"), 1));
...
//    strUsage += "  -anonymizesecuryptoamount=<n>     " + strprintf(_("Keep N SCU anonymized (default: %u)"), 0) + "\n";
...
/** Sanity checks
 *  Ensure that Securypto is running in a usable environment with all
...
bool AppInit2()
{
	...
	filesystem::path checkExist = GetDataDir() / "blocks";
 	filesystem::path chainFile = GetDataDir() / "chainversion3";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Securypto Core is shutting down."));
...
#endif
    // Make sure only a single Securypto process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Securypto Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Securypto version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //Securypto: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Securypto specific: zerocoin and spork DB's
                ...
                // Securypto: load previous sessions sporks if we have them.
...
*/
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainHeight > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // Securypto: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Securypto Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Securypto Core to complete") << "\n";
...
//    }
//
//    nAnonymizeSecuryptoAmount = GetArg("-anonymizesecuryptoamount", 0);
//    if (nAnonymizeSecuryptoAmount > 999999) nAnonymizeSecuryptoAmount = 999999;
//    if (nAnonymizeSecuryptoAmount < 2) nAnonymizeSecuryptoAmount = 2;
    ...
    LogPrintf("Anonymize Securypto Amount %d\n", nAnonymizeSecuryptoAmount);
    ...
    */
    ...
    if (pwalletMain) {
        ...
        if (GetBoolArg("-precompute", true)) {
            // Run a thread to precompute any zSCU spends
