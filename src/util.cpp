...
int nAnonymizeSecuryptoAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "securypto" is a composite category enabling all Securypto-related debug output
            if (ptrCategory->count(string("securypto"))) {
...
#else
    const char* pszModule = "securypto";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "securypto.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty securypto.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override securypto.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "securyptod.pid"));
