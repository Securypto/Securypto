Securypto Core version *3.1.0* is now available from:  <https://github.com/securypto-project/securypto/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/securypto-project/securypto/issues>


Mandatory Update
==============

Securypto Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zSCU Staking is here! zSCU staking will be activated on the morning of the 8th of May 2018. With the release of zSCU staking, there are effectively 2 versions of zSCU, zSCU minted on the 3.0.6 Securypto wallet or lower, and zSCU minted on Securypto wallet version or higher. New features in this release will require the use of zSCU v2, zSCU minted on this wallet release 3.1.0 or later. If you currently hold zSCU v1 and wish to take advantage of zSCU staking and deterministic zSCU, you will need to spend the zSCU v1 to yourself and remint zSCU v2.
...
SCU/zSCU Staking and Masternode Rewards
--------------

### SCU, zSCU and Masternode Payment Schedule

To encourage the use of zSCU and increase the Securypto zerocoin anonymity set, the Securypto payment schedule has been changed to the following:

If a user staking zSCU wins the reward for their block, the following zSCU reward will be: 
- 3 zSCU (3 x 1 denominations) rewarded to the staker, 2 SCU rewarded to the masternode owner and 1 SCU available for the budget. This is a total block reward of 6 SCU, up from 5.

If a user staking SCU wins the reward, the following amounts will be rewarded: 
- 2 SCU to the SCU staker, 3 SCU to the Masternode owner and 1 SCU available for the budget. This is a total block reward of 6 SCU, up from 5.


### Return change to sender when minting zSCU

Previously, zSCU minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most SCU and return the change from a zSCU mint to that address.
...
The visual layout of the Securypto Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zSCU minting feature. This is enabled by default and the enablezeromint=0 setting in the securypto.conf file will overwrite the GUI option.
-	The percentage of autominted zSCU can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular SCU.
...
### Summary
 ...
 The Securypto wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the securypto.conf file, found in the Securypto data directory.
...
Securypto Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The Securypto client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running securyptod without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the Securypto support group, which often times sees users say "I didn't spend that zSCU". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the Securypto masternodes, the monthly budget available to be utilised has been increased to 42,000 SCU / month. This SCU only has the opportunity to be raised once per month (paid to winning proposals) with any unused SCU not created by the blockchain.

### Budget Finalization Fee

The Securypto finalization fee for successful proposals has now been reduced, this fee is now 5 SCU down from 50 SCU. The total fee outlay for a successful proposal is now a total of 55 SCU.
...
-	Fixed a  bug that would not start masternodes from the Securypto-Qt masternodes tab
-	Updated Securypto-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and Securypto's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zSCU Control dialog in the QT wallet, a user is able to see zSCU marked as version 1 or 2.
...
Zerocoins, or zSCU, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zSCU that can be recalculated at any time using the seed. Deterministic zSCU allows for users to backup all of their future zSCU by simply recording their seed and keeping it in a safe place (similar to backing up a private key for SCU). The zSCU seed needs to remain in the wallet in order to spend the zSCU after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zSCU data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

Securypto's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which Securypto used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to Securypto using the bug bounty program. The modulus is now correctly imported and Securypto's accumulators have been changed to use the new proper modulus.
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault securypto-cli getinfo while loading block index (Mrs-X)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/securypto-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
